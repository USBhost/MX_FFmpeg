...
static const uint64_t ONE_SECOND		= 1000000000LL;	// in n-sec.

/**
 * @return uptime in u-sec.
 */
static int64_t uptime()
{
    ...
    if( clock_gettime(CLOCK_MONOTONIC, &t) == 0 )
    	return t.tv_sec * ONE_SECOND + t.tv_nsec / 1000;
    else
    	return 0;
}
...
//	if( nb_colors_found == nb_opaque_colors )		--> 실제로 entry가 누락되었어도 사용되지 않던 color가 사용되는 경우는 거의 없다.
	...
	{
		struct Entry* newEntries = realloc( ctx->entries, sizeof(struct Entry) * (ctx->nb_entry+1) );
		if( newEntries == NULL )
		{
	...
	DBG( "DVDSubContext2 - %dms were spent to guess dvdsub palette with colormap %d %d %d %d / alpha %d %d %d %d. opaque-colors=%d found-colors=%d",
			(int)((uptime()-begin)/1000), colormap[0], colormap[1], colormap[2], colormap[3], alpha[0], alpha[1], alpha[2], alpha[3], nb_opaque_colors, nb_colors_found );
}


struct DVDSubContext2* dvdsub2_init()
{
	struct DVDSubContext2* ctx = calloc( 1, sizeof(struct DVDSubContext2) );
