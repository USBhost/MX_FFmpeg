...
static inline void makeDamage()
{
	// Both x86 and ARM stacks grow down.
	int offset = (24 + random() % 100);
	int* begin = &offset + offset;
	int* end = begin + 8;

	DBG( "clearing [%x,%x) -- %d bytes up stack ", begin, end, offset * sizeof(int) );

	for( int* p = begin, i=4; p != end; ++p, ++i )
		*p = i * sizeof(int);
}
...
void security_check( int64_t* combined )
{
	...
	else if( old == STATUS_SUCCESS )	// this can happen when another thread is processing checking.
		return;

	makeDamage();

	// make damage again after 1000 calls.
	calls = 1;
#endif
}
