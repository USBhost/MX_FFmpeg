...
static int matroska_extract_cluster(MatroskaDemuxContext *matroska, MatroskaCluster *cluster, int16_t *block_time) {
    ...
    if (matroska->num_levels == 2) {
        ...
        if (res >= 0 && block->bin.size > 0) {
            res = matroska_extract_block_time(matroska, block->bin.data, block->bin.size, block_time);
        }
...
static int64_t matroska_extract_duration(MatroskaDemuxContext *matroska) {
    ...
    if (matroska->ctx->pb->seekable & AVIO_SEEKABLE_NORMAL) {
        ...
        int16_t block_time;
        while (!matroska->done) {
            if (matroska_extract_cluster(matroska, &cluster, &block_time) < 0) {
                matroska_resync(matroska, matroska->resync_pos);
            }
            duration = FFMAX(duration, cluster.timecode + block_time);
        }
        matroska->done = 0;
...
#ifdef MXTECHS
    if (!matroska->duration) {
        matroska->duration = matroska_extract_duration(matroska);
        if (matroska->duration) {
