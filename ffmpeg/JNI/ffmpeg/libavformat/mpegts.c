...
static char *getstr8(const uint8_t **pp, const uint8_t *p_end)
{
    ...
    if (len > p_end - p)
        return NULL;
/*
 * iconv(GPL) is implemented with icu because of license issue.
 * iconv methods are unavailable until iconv_connect_client called.
 * Currently, iconv_connect_client is not called in service mode.
 * TODO:Thus, simply comment out iconv methods calling until really necessary.
 */
#ifndef MXTECHS
#if CONFIG_ICONV
    if (len) {
